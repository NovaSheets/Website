...
function parse(content, novasheets = new novasheets_1.default()) {
    ...
    for (let loop = 0, lastCssOutput; loop < 1 || loop < constants.MAX_RECURSION && hasNovaSheetsStyles(); loop++) {
        ...
        const tokenized = [{ name: 'Root', content: '', body: [] }];
        let content = '';
        let tokenTree = [tokenized[0]];
        let selectorTree = [''];
        ...
        for (let i = 0; i < cssOutput.length; i++) {
            ...
            if (char === '{') {
                ...
                const parentSelector = selectorTree[selectorTree.length - 2] || '';
        ...
        // clear parsed blocks
        for (let regex = /[^{}]+{[^{}]*}/gs; regex.test(cssOutput);) {
            cssOutput = cssOutput.replace(regex, '');
        }
        // move all sub-blocks to root
        let blocks = [];
        const flatten = (obj) => {
            for (let o of obj === null || obj === void 0 ? void 0 : obj.body) {
                if (o.name === 'Style')
        ...
        flatten(tokenTree[0]);
        // create unnested CSS
        let flattenedCssOutput = cssOutput;
