...
function parse(content, novasheets = new novasheets_1.default()) {
    ...
    const SLASH = Math.random().toString(36).substr(2);
    ...
    for (let loop = 0, lastCssOutput; loop < 1 || loop < constants.MAX_RECURSION && hasNovaSheetsStyles(); loop++) {
        ...
        cssOutput = cssOutput
            ...
            .replace(/((?:rgba?|hsla?)\(.+?[\s\d%]+)\/([\s\d%]+\))/g, (_, before, after) => before + SLASH + after);
        for (let i = 0; i < constants.MAX_MATH_RECURSION; i++) {
            ...
            cssOutput = cssOutput.replace(RegExp(mathChecker, 'g'), mathMatch => {
                const isDelimited = /\d\s+-\d/.test(mathMatch);
                const matchesOnlyBrackets = !/[-+^*/]/.test(mathMatch);
                const containsUnitList = mathMatch.match(RegExp(r `${numberUnit}\s-?${basedNumber}`));
                if (isDelimited || matchesOnlyBrackets || containsUnitList)
                    return mathMatch;
