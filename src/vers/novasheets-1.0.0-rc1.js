    ...
    for (let loop = 0, lastCssOutput; loop < 1 || loop < constants.MAX_RECURSION && hasNovaSheetsStyles(); loop++) {
        ...
        for (let i = 0; i < cssOutput.length; i++) {
            ...
            if (char === '{') {
                ...
                const parentSelector = selectorTree.slice(0, selectorTree.length - 1).join(' ');
        ...
        let blocks = [];
        const flatten = (obj) => {
            for (const o of obj === null || obj === void 0 ? void 0 : obj.body) {
                if (o.name === 'Style')
        ...
        flatten(tokenTree[0]);
        // create unnested CSS
        let flattenedCssOutput = passthroughContent + cssOutput;
