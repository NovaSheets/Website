...
function parse(content, novasheets = new novasheets_1.default()) {
    ...
    for (let loop = 0, lastCssOutput; loop < 1 || loop < constants.MAX_RECURSION && hasNovaSheetsStyles(); loop++) {
        ...
        // Parse math //
        const exponentMatcher = r `(?<!#|\w)(${number})\s*e\s*([+-]?${number})`;
        ...
        for (let i = 0; i < constants.MAX_MATH_RECURSION; i++) {
            ...
            cssOutput = cssOutput.replace(RegExp(mathChecker, 'g'), mathMatch => {
                const matchContent = cssOutput.slice(0, cssOutput.indexOf(mathMatch) + mathMatch.length) + ')';
                const isCssFunc = /\w+\(.+?\)/.test(matchContent); // avoid treating '/' in CSS color functions as division
                const isDelimited = /\d\s+-\d/.test(mathMatch); // avoid cases like '0 -2em'
                if (isCssFunc || isDelimited)
                    return mathMatch;
                ...
                if (matchesOnlyBrackets || containsUnitList)
                    return mathMatch;
                let numMatch = Array.from(mathMatch.match(RegExp(numberUnit, 'g')) || []);
        ...
        for (let i = 0; i < cssOutput.length; i++) {
            ...
            if (char === '{') {
                ...
                const parentSelector = ((_b = selectorTree.slice(-1)[0]) === null || _b === void 0 ? void 0 : _b.replace(/\/\*.+?\*\//g, '')) || '';
